...
class AdvocateKeywordEvaluator:
    def __init__(self):
        ...
        messages = [
            {
                'role': 'system',
                'content': (
                    "You are an expert in legal language analysis and advocate statement comprehension. "
                    "You will identify key legal terms, phrases, or concepts from advocate statements.\n\n"

                    "Your task is to:\n"
                    "1. Analyze the case context and dialogue background\n"
                    "2. Focus on the specific advocate statement provided\n"
                    "3. Identify which legal concept, term, or phrase is most relevant to the advocate's argument\n"
...
def save_overall_accuracy_summary(output_dir: str, model_name: str, all_results: dict):
    ...
    try:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write(f"整体律师关键词识别准确率摘要 ({model_name.upper()}模型)\n")
            f.write("=" * 80 + "\n")
...
def main():
    ...
    evaluator = AdvocateKeywordEvaluator()
    ...
    for json_file in json_files:
        ...
        # Clean the base name to remove "advocate_keywords_" prefix if present
        if base_name.startswith("advocate_keywords_"):
            base_name = base_name[18:]  # Remove "advocate_keywords_" prefix
